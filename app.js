document.addEventListener("DOMContentLoaded", function () {
  let quizCompleted = false; // Flag to track whether the quiz has been completed
  let timerInterval; // Variable to store the timer interval
  let timeLimit = 15 * 60; // 15 minutes in seconds
  ...
  function startQuiz(event) {
    ...
    const userUSN = usnInput.value;

    document.getElementById("nameForm").style.display = "none";
    document.getElementById("greeting").innerText = `Hi, ${userName}! Best of Luck!!`;
    document.getElementById("quiz").style.display = "block";

    // Start the timer only if the quiz is not completed
    if (!quizCompleted) {
      startTimer();
    }
  }
  ...
  function startTimer() {
    ...
    timerInterval = setInterval(function () {
      ...
      document.getElementById("timeSpan").textContent = `Time Left: ${minutes}m ${seconds}s`;

      if (timeLimit <= 0) {
        // Stop the timer and redirect to the "end.html" page
